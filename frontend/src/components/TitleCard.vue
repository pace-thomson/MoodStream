        ...
        <div class="card-body" @click="openModal(show)">
          <div class="card-info-flex">
            <div class="card-text-content">
              ...
              <p class="card-year">{{ show.releaseYear }}</p>
  ...
  <div class="results-container">
    ...
    <div class="results-grid">
      ...
      >
        ...
        <button class="add-watchlist-button" @click="addToWatchlist(show.imdbId)">+</button>
        ...
        <div class="card-body" @click="openModal(show)">
          <div class="card-info-flex">
            ...
            <a :href="getFirstStreamingLink(show)" @click.stop target="_blank" class="play-button">
              <div class="play-button-icon">&#9658;</div>
            </a>
          </div>
        </div>
    ...
    <div v-if="selectedShow" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        ...
        <div class="modal-header">
          ...
          <div class="modal-header-info">
            <h1>{{ selectedShow.title }}</h1>
            <p class="modal-year">{{ selectedShow.releaseYear }}</p>
            <div class="genres">
              <span v-for="genre in selectedShow.genres" :key="genre.id" class="genre-tag">
                {{ genre.name }}
              </span>
            </div>
          </div>
...
// --- Computed property to filter streaming options for the modal ---
const filteredStreamingOptions = computed(() => {
  if (!selectedShow.value || !selectedShow.value.streamingOptions.us) {
    return [];
  }
  const allowedTypes = ['subscription', 'free'];
  return selectedShow.value.streamingOptions.us.filter(option => 
    allowedTypes.includes(option.type)
  );
});
...
function addToWatchlist(imdbId) {
  if (!userWatchlist.value.includes(imdbId)) {
    userWatchlist.value.push(imdbId);
    console.log('Added to watchlist:', imdbId);
  } else {
    console.log('Already in watchlist:', imdbId);
  }
}

// --- Helper Functions ---

// Gets the first available 'subscription' or 'free' link for the main card's play button.
function getFirstStreamingLink(show) {
  if (!show.streamingOptions.us) {
    return '#';
  }
  
  const allowedTypes = ['subscription', 'free'];
  const firstAvailableLink = show.streamingOptions.us.find(option =>
    allowedTypes.includes(option.type)
  );
  
  return firstAvailableLink ? firstAvailableLink.link : '#';
}

// Gets the best available poster, preferring the highest resolution available.
function getPosterUrl(show) {
  // Use optional chaining (?.) to safely access the nested poster object
  const poster = show?.imageSet?.horizontalPoster;

  if (poster) {
    // Return the best available resolution, checking from highest to lowest.
    return poster.w1440 || poster.w1080 || poster.w720 || poster.w480 || poster.w360;
  }
  
  // If no horizontal poster object is found, return the placeholder.
  return 'https://placehold.co/780x439/2d3748/ffffff?text=Poster+Not+Found';
}
