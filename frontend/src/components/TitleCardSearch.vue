      ...
      <div v-if="selectedShow" class="modal-overlay" @click.self="closeModal">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <h2>Available On</h2>
            <div class="streaming-options-container">
              <div v-if="allStreamingOptions.length > 0" class="streaming-options">
                <div 
                  v-for="option in allStreamingOptions" 
                  :key="option.service.id"
                  class="streaming-option-item"
                >
                  <a 
                    :href="option.link" 
                    target="_blank"
                  ...
                  <span v-if="option.displayLabel" class="streaming-type-label">{{ option.displayLabel }}</span>
                </div>
              </div>
              <div v-else>
                <p>No streaming options found in your region.</p>
              </div>
            </div>
          </div>
        </div>
...
// --- Computed Properties ---
const allStreamingOptions = computed(() => {
    if (!selectedShow.value || !selectedShow.value.streamingOptions.us) {
        return [];
    }
    
    const userSubscriptions = new Set(props.catalogs);
    
    const uniqueOptions = selectedShow.value.streamingOptions.us.reduce((acc, option) => {
        const serviceId = option.service.id;
        if (!acc.has(serviceId)) {
            let displayLabel = null;
            // Always show a label for non-subscription/non-free types
            if (option.type !== 'subscription' && option.type !== 'free') {
                displayLabel = option.type.charAt(0).toUpperCase() + option.type.slice(1);
            } 
            // Only show 'Subscription' label if user is NOT subscribed
            else if (option.type === 'subscription' && !userSubscriptions.has(serviceId)) {
                displayLabel = 'Subscription';
            }
            
            acc.set(serviceId, { ...option, displayLabel });
        }
        return acc;
    }, new Map());
...
async function handleAddToWatchlist(show) {
    ...
    if (error) {
        showAlert(error.message === 'already_exists' ? `${show.title} is already in your watchlist.` : `Could not add ${show.title} to watchlist.`);
    } else {
...
function getPlayableLink(show) {
    if (!show.streamingOptions.us) return null;
    const playableOption = show.streamingOptions.us.find(option => {
        if (option.type === 'subscription' && props.catalogs.includes(option.service.id)) {
            return true;
        }
        return option.type === 'free';
    });
...
<style scoped>
/* All existing styles from TitleCard.vue are copied here */
.results-grid {
    display: grid;
...
/* UPDATED: Styles for the streaming type label */
.streaming-type-label {
    font-size: 0.8rem;
    font-weight: 600;
    color: #e2e8f0;
    background-color: #4a5568; /* A neutral blue-gray */
